...
void handleLoraRx(String hex) {
  ...
  if (hex.length() < 6) {  // need at least device + service + value = 6 hex chars
    Serial.println("[LORA RX] Payload too short, ignoring");
    return;
  }

  uint8_t rxDeviceId = (uint8_t)strtol(hex.substring(0, 2).c_str(), nullptr, 16);
  if (rxDeviceId != DEVICE_ID) {
    Serial.printf("[LORA RX] Not for us (got %02X, ours %02X)\n", rxDeviceId, DEVICE_ID);
    return;
  }
  // Now we have made sure that the message is for us
  ...
  switch (service) {
    ...
    case 0x04:  // LED
      if      (value == 0x01) { Serial.println("[LORA RX] CMD: LED_BLUE");  sendCurtainCommand("LED_BLUE"); }
      else if (value == 0x02) { Serial.println("[LORA RX] CMD: LED_RED");   sendCurtainCommand("LED_RED");  }
      else                    { Serial.println("[LORA RX] CMD: LED_OFF");   sendCurtainCommand("LED_OFF");  }
      lastAppliedMode = "MANUAL";
      break;

    case 0x05:  // Mode override
      if (value == 0x01) {
...
void loraTask(void* parameter) {
  ...
  while (true) {
    ...
    // Open RX window (e.g., 5 seconds)
    loraSerial.println("radio rx 5000");
    str = loraSerial.readStringUntil('\n'); // "ok" or "busy"
    if (str.indexOf("ok") >= 0) {
      str = loraSerial.readStringUntil('\n'); // "radio_rx  <hex>" or "radio_err"
      if (str.startsWith("radio_rx")) {
        String rxPayload = str.substring(str.indexOf("  ") + 2);
        rxPayload.trim();
        Serial.println("[LORA RX] " + rxPayload);
        handleLoraRx(rxPayload); // your handler
        }
      }
    vTaskDelay(100 / portTICK_PERIOD_MS);
    }
